"""Walton County (FL) Property Appraiser — SCAFFOLD (live probe pending).

Part of the Landmark next-batch wave. Recorder adapter + per-county recorder
config + tax recipe are already in place for Walton; this is the missing
Property Appraiser anchor (Broward-Standard item #1).

Best-known portal (VERIFY ON PROBE): https://www.waltonpa.com/
...
to ``walton_pa_http``, add a dedicated factory branch + real unit tests.
"""
...
class WaltonPA(LandmarkPAScaffold):
    SOURCE_LABEL = "Walton County Property Appraiser"
    LIVE_PLATFORM = "walton_pa_http"
