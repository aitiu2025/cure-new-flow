"""Flagler County (FL) Property Appraiser — SCAFFOLD (live probe pending).

Part of the Landmark next-batch wave. Recorder adapter + per-county recorder
config + tax recipe are already in place for Flagler; this is the missing
Property Appraiser anchor (Broward-Standard item #1).

Best-known portal (VERIFY ON PROBE): https://www.flaglerpa.com/
...
to ``flagler_pa_http``, add a dedicated factory branch + real unit tests.
"""
...
class FlaglerPA(LandmarkPAScaffold):
    SOURCE_LABEL = "Flagler County Property Appraiser"
    LIVE_PLATFORM = "flagler_pa_http"
