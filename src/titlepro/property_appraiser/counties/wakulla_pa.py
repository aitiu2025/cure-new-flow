"""Wakulla County (FL) Property Appraiser — SCAFFOLD (live probe pending).

Part of the Landmark next-batch wave. Recorder adapter + per-county recorder
config + tax recipe are already in place for Wakulla; this is the missing
Property Appraiser anchor (Broward-Standard item #1).

Best-known portal (VERIFY ON PROBE): https://www.mywakullapa.com/
...
to ``wakulla_pa_http``, add a dedicated factory branch + real unit tests.
"""
...
class WakullaPA(LandmarkPAScaffold):
    SOURCE_LABEL = "Wakulla County Property Appraiser"
    LIVE_PLATFORM = "wakulla_pa_http"
