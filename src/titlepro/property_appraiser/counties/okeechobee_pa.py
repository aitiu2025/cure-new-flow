"""Okeechobee County (FL) Property Appraiser — SCAFFOLD (live probe pending).

Part of the Landmark next-batch wave. Recorder adapter + per-county recorder
config + tax recipe are already in place for Okeechobee; this is the missing
Property Appraiser anchor (Broward-Standard item #1).

Best-known portal (VERIFY ON PROBE): https://www.okeechobeepa.com/
...
to ``okeechobee_pa_http``, add a dedicated factory branch + real unit tests.
"""
...
class OkeechobeePA(LandmarkPAScaffold):
    SOURCE_LABEL = "Okeechobee County Property Appraiser"
    LIVE_PLATFORM = "okeechobee_pa_http"
