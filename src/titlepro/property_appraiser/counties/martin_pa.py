"""Martin County (FL) Property Appraiser — SCAFFOLD (live probe pending).

Part of the Landmark next-batch wave. Recorder adapter + per-county recorder
config + tax recipe are already in place for Martin; this is the missing
Property Appraiser anchor (Broward-Standard item #1).

Best-known portal (VERIFY ON PROBE): https://www.pa.martin.fl.us/
...
to ``martin_pa_http``, add a dedicated factory branch + real unit tests.
"""
...
class MartinPA(LandmarkPAScaffold):
    SOURCE_LABEL = "Martin County Property Appraiser"
    LIVE_PLATFORM = "martin_pa_http"
