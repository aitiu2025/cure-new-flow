"""Indian River County (FL) Property Appraiser — SCAFFOLD (live probe pending).

Part of the Landmark next-batch wave. Recorder adapter + per-county recorder
config + tax recipe are already in place for Indian River; this is the missing
Property Appraiser anchor (Broward-Standard item #1).

Best-known portal (VERIFY ON PROBE): https://www.ircpa.org/
...
to ``indian_river_pa_http``, add a dedicated factory branch + real unit tests.
"""
...
class IndianRiverPA(LandmarkPAScaffold):
    SOURCE_LABEL = "Indian River County Property Appraiser"
    LIVE_PLATFORM = "indian_river_pa_http"
