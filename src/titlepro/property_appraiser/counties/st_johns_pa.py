"""St. Johns County (FL) Property Appraiser — SCAFFOLD (live probe pending).

Part of the Landmark next-batch wave. Recorder adapter + per-county recorder
config + tax recipe are already in place for St. Johns; this is the missing
Property Appraiser anchor (Broward-Standard item #1).

Best-known portal (VERIFY ON PROBE): https://www.sjcpa.us/
...
to ``st_johns_pa_http``, add a dedicated factory branch + real unit tests.
"""
...
class StJohnsPA(LandmarkPAScaffold):
    SOURCE_LABEL = "St. Johns County Property Appraiser"
    LIVE_PLATFORM = "st_johns_pa_http"
