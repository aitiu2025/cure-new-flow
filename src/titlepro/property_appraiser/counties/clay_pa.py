"""Clay County (FL) Property Appraiser — SCAFFOLD (live probe pending).

Part of the Landmark next-batch wave. Recorder adapter + per-county recorder
config + tax recipe are already in place for Clay; this is the missing
Property Appraiser anchor (Broward-Standard item #1).

Best-known portal (VERIFY ON PROBE): https://www.ccpao.com/
...
to ``clay_pa_http``, add a dedicated factory branch + real unit tests.
"""
...
class ClayPA(LandmarkPAScaffold):
    SOURCE_LABEL = "Clay County Property Appraiser"
    LIVE_PLATFORM = "clay_pa_http"
