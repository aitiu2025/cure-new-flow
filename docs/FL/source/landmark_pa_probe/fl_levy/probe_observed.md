# Levy County PA — live probe (observed 2026-06-17)
...
## Portal
- County landing: `https://www.qpublic.net/fl/levy/` — a **WordPress** marketing site (no property data of
  its own). All record/parcel search links delegate OUT to Schneider qPublic.
- **Real search engine:** `https://qpublic.schneidercorp.com/Application.aspx?AppID=930&PageType=Search`
- Platform: **qPublic by Schneider Geospatial** (a.k.a. "Beacon" / `qpublic.schneidercorp.com`).
  Multi-tenant **ASP.NET WebForms** app keyed by `AppID=930` (also accepts the
  numeric `AppID=930` directly — confirmed loads identically).
...
- Single page `Application.aspx?AppID=930&PageType=Search` exposes multiple search
...
## Adapter implication
Do NOT build a bespoke Levy adapter. This county is covered by the shared
**`qpublic_schneider_pa_http`** adapter, parameterized by `app_id=930`
(`AppID=930`). Mirror template = **Escambia `escambia_pa.py`** (same ASP.NET
...
- search URL: `https://qpublic.schneidercorp.com/Application.aspx?AppID=930&PageType=Search`
