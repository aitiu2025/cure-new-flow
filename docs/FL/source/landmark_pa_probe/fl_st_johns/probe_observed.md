# St. Johns County PA — live probe (observed 2026-06-17)
...
## Portal
- County landing: `https://www.sjcpa.gov/` — a **WordPress** marketing site (no property data of
  its own). All record/parcel search links delegate OUT to Schneider qPublic.
- **Real search engine:** `https://qpublic.schneidercorp.com/Application.aspx?App=StJohnsCountyFL&PageType=Search`
- Platform: **qPublic by Schneider Geospatial** (a.k.a. "Beacon" / `qpublic.schneidercorp.com`).
  Multi-tenant **ASP.NET WebForms** app keyed by `App=StJohnsCountyFL` (also accepts the
  numeric `AppID=960` directly — confirmed loads identically).
...
- Single page `Application.aspx?App=StJohnsCountyFL&PageType=Search` exposes multiple search
...
## Adapter implication
Do NOT build a bespoke St. Johns adapter. This county is covered by the shared
**`qpublic_schneider_pa_http`** adapter, parameterized by `app_id=960`
(`App=StJohnsCountyFL`). Mirror template = **Escambia `escambia_pa.py`** (same ASP.NET
...
- `app_id`: `960`
- friendly `App=`: `StJohnsCountyFL`
- search URL: `https://qpublic.schneidercorp.com/Application.aspx?App=StJohnsCountyFL&PageType=Search`
