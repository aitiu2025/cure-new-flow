# Indian River County PA — live probe (observed 2026-06-17)
...
## Portal
- County landing: `https://www.ircpa.org/` — a **WordPress** marketing site (no property data of
  its own). All record/parcel search links delegate OUT to Schneider qPublic.
- **Real search engine:** `https://qpublic.schneidercorp.com/Application.aspx?App=IndianRiverCountyFL&PageType=Search`
- Platform: **qPublic by Schneider Geospatial** (a.k.a. "Beacon" / `qpublic.schneidercorp.com`).
  Multi-tenant **ASP.NET WebForms** app keyed by `App=IndianRiverCountyFL` (also accepts the
  numeric `AppID=1109` directly — confirmed loads identically).
- County landing site served plain (no Cloudflare on the WordPress shell). The qPublic search app is Cloudflare-fronted (`server: cloudflare`) — use `curl_cffi` `safari17_2_ios`.
...
- Single page `Application.aspx?App=IndianRiverCountyFL&PageType=Search` exposes multiple search
...
## Adapter implication
Do NOT build a bespoke Indian River adapter. This county is covered by the shared
**`qpublic_schneider_pa_http`** adapter, parameterized by `app_id=1109`
(`App=IndianRiverCountyFL`). Mirror template = **Escambia `escambia_pa.py`** (same ASP.NET
...
- `app_id`: `1109`
- friendly `App=`: `IndianRiverCountyFL`
- search URL: `https://qpublic.schneidercorp.com/Application.aspx?App=IndianRiverCountyFL&PageType=Search`
