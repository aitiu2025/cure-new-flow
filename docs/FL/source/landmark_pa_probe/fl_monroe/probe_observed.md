# Monroe County PA — live probe (observed 2026-06-17)
...
## Portal
- County landing: `https://www.mcpafl.org/` — a **WordPress** marketing site (no property data of
  its own). All record/parcel search links delegate OUT to Schneider qPublic.
- **Real search engine:** `https://qpublic.schneidercorp.com/Application.aspx?AppID=605&PageType=Search`
- Platform: **qPublic by Schneider Geospatial** (a.k.a. "Beacon" / `qpublic.schneidercorp.com`).
  Multi-tenant **ASP.NET WebForms** app keyed by `AppID=605` (also accepts the
  numeric `AppID=605` directly — confirmed loads identically).
...
- Single page `Application.aspx?AppID=605&PageType=Search` exposes multiple search
...
## Adapter implication
Do NOT build a bespoke Monroe adapter. This county is covered by the shared
**`qpublic_schneider_pa_http`** adapter, parameterized by `app_id=605`
(`AppID=605`). Mirror template = **Escambia `escambia_pa.py`** (same ASP.NET
...
- search URL: `https://qpublic.schneidercorp.com/Application.aspx?AppID=605&PageType=Search`
